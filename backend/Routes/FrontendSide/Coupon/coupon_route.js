...
route.post('/createbyadmin', async (req, res) => {
    try {
        const { couponCode, creationDate, expiryDate, usageLimits, discountAmount, userType, selectedUser, discountType, discountPercent, maxDiscount, minAmount } = req.body;
...
route.get('/get', async (req, res) => {
    try {
        ...
        const populatedCoupons = coupons.map(coupon => {
            ...
            return {
                ...coupon.toObject(),
                creationDate: new Date(coupon?.creationDate)?.toLocaleDateString('en-IN'),
